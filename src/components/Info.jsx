...
import { loadingNeighbors } from "../store/details/details-actions";
...
export const Info = (props)=>{
    ...
    useEffect(()=>{
       if(borders.length){
            dispatch(loadingNeighbors(borders))
       } 
    },[borders,dispatch])
    return(
        <Wrapper>
            ...
            <div>
                ...
                <Meta>
                    ...
                    {!borders.length
                    ? (<span>Not have border countries</span>)
                    :(<TagGroup>
                        {neighbors.map(country=>(<Tag key={country} onClick={()=>push(`/country/${country}`)}>{country}</Tag>))}
                    </TagGroup>)}
                </Meta>
            </div>
        </Wrapper>
